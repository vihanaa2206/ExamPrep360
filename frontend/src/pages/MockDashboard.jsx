  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col"
        style={{ animation: "zoomIn .25s ease" }} onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center gap-6 px-6 py-4 border-b border-gray-100 flex-shrink-0">
          ...
          <div className="grid grid-cols-3 gap-4 flex-1">
            <div className="text-center"><p className="text-2xl font-black text-green-600">{correct}</p><p className="text-xs text-gray-400">Correct</p></div>
            <div className="text-center"><p className="text-2xl font-black text-red-500">{wrong}</p><p className="text-xs text-gray-400">Wrong</p></div>
            <div className="text-center"><p className="text-2xl font-black text-gray-400">{skipped}</p><p className="text-xs text-gray-400">Skipped</p></div>
          </div>
          ...
          {answers.length === 0 ? (
            ...
            <div className="grid md:grid-cols-2 gap-3">
              {answers.map((a, i) => {
                ...
                return (
                  <div key={i} className={`rounded-2xl border-2 p-4 text-sm shadow-sm ${isCorrect?"bg-green-50 border-green-400":isWrong?"bg-red-50 border-red-400":"bg-gray-100 border-gray-300"}`}>
                    <div className="flex items-start justify-between gap-2 mb-3">
                      <p className="font-bold text-gray-900 leading-snug line-clamp-3">Q{i+1}. {a.question_text || `Question ${i+1}`}</p>
                      <span className={`flex-shrink-0 px-2.5 py-1 rounded-full font-black text-xs whitespace-nowrap ${isCorrect?"bg-green-500 text-white":isWrong?"bg-red-500 text-white":"bg-gray-500 text-white"}`}>
                    ...
                    <div className={`rounded-xl px-3 py-2 mb-2 ${isCorrect?"bg-green-100":isWrong?"bg-red-100":"bg-gray-200"}`}>
                      ...
                      <p className={`font-black text-base mt-0.5 ${isCorrect?"text-green-800":isWrong?"text-red-800":"text-gray-600"}`}>{a.selected_option || "Not answered"}</p>
                    </div>
...
function LeaderboardPopup({ exam, allHistory, currentUserId, onClose }) {
  const userMap = {};
  allHistory.filter(h => h.exam === exam).forEach(h => {
    ...
    if (!userMap[uid] || score > userMap[uid].score) userMap[uid] = { uid, name, score, maxScore, accuracy };
  });
  const board = Object.values(userMap).sort((a, b) => b.score - a.score || b.accuracy - a.accuracy);
  const medalEmoji = (i) => i===0?"🥇":i===1?"🥈":i===2?"🥉":null;
  const rankBg = (i) => i===0?"bg-yellow-50 border-yellow-300":i===1?"bg-slate-50 border-slate-300":i===2?"bg-orange-50 border-orange-300":"bg-white border-gray-100";

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      style={{ background: "rgba(0,0,0,0.65)", backdropFilter: "blur(4px)" }} onClick={onClose}>
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md max-h-[85vh] overflow-hidden flex flex-col"
        style={{ animation: "zoomIn .25s ease" }} onClick={e => e.stopPropagation()}>
            ...
            return (
              <div key={u.uid} className={`rounded-2xl border-2 p-4 flex items-center gap-4 transition ${rankBg(i)} ${isMe?"ring-2 ring-blue-400":""}`}>
                <div className="w-10 text-center flex-shrink-0">
                  {medalEmoji(i)?<span className="text-2xl">{medalEmoji(i)}</span>:<span className="text-lg font-black text-gray-400">#{i+1}</span>}
                </div>
                ...
                <div className="flex-shrink-0 text-right">
                  <p className={`text-2xl font-black ${u.accuracy>=70?"text-green-600":u.accuracy>=50?"text-yellow-600":"text-red-500"}`}>{u.accuracy}%</p>
...
export default function MockDashboard() {
  ...
  useEffect(() => {
    ...
    // ✅ FIX: handle MongoDB $oid format
    const fetchId = currentUser._id?.$oid || currentUser._id || currentUser.id;
    const uid = fetchId || currentUser.email;
    setCurrentUserId(uid);

    if (fetchId) {
      fetch(`${API}/reports/user/${fetchId}`)
        .then(r => r.json())
        .then(data => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {leaderboardExam && <LeaderboardPopup exam={leaderboardExam} allHistory={allHistory} currentUserId={currentUserId} onClose={()=>setLeaderboardExam(null)}/>}
        ...
        {activeTab==="history" && (
          <div className="space-y-3">
            ...
            {history.map((h,i)=>{
              ...
              return (
                <div key={i} className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm hover:shadow-lg transition cursor-pointer hover:scale-[1.01]"
                  style={{transition:"all .2s ease"}} onClick={()=>setSelectedTest(h)}>
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-400 mb-2">Test {h.testNo} · {h.scheme} · {h.date?new Date(h.date).toLocaleDateString("en-IN",{day:"2-digit",month:"short",year:"numeric"}):"—"}</p>
        ...
        {activeTab==="leaderboard" && (
          <div className="space-y-3">
            ...
            {examList.map(e=>{
              ...
              return (
                <div key={e.exam} className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm hover:shadow-lg cursor-pointer hover:scale-[1.01] transition"
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {board.slice(0,3).map((u,i)=>(
                      <div key={u.uid} className={`flex items-center gap-1.5 rounded-xl px-3 py-1.5 text-xs border ${i===0?"bg-yellow-50 border-yellow-200":i===1?"bg-gray-50 border-gray-200":"bg-orange-50 border-orange-200"}`}>
                        <span>{medal(i)}</span>
